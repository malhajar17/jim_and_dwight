...
export class EnrichNode {
  constructor() {
    ...
    // Jina API configuration with provided credentials
    this.jinaApiKey = 'jina_c0591c5718424dd58335d8f7c639968d5UhgcZuGIoxMJ7TVl5_f81xDezeh';
  ...
  async generatePersonalitySummary(name, websiteContents) {
    try {
      ...
      const combinedContent = websiteContents.join('\n\n---\n\n');
...
${combinedContent}`;
  ...
  async enrichLeadWithPersonalityAnalysis(lead) {
    try {
      ...
      } // Close the conditional scraping block
  ...
  async run(msg, ctx) {
    try {
      ...
      const leadsToEnrich = state.leads
        .filter(lead => !lead.personality_analysis) // Only enrich leads that haven't had personality analysis
      ...
      if (leadsToEnrich.length === 0) {
        console.log('✅ All leads already have personality analysis!');

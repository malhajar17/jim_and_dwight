  ...
  async run(msg, ctx) {
    try {
      ...
      if (!this.enricher.isConfigured()) {
        ...
        return this.createNextMessage(msg.run_id, fallbackPersonas);
      }
      ...
      // Create next message
      return this.createNextMessage(msg.run_id, personas);
...
Respond with ONLY a JSON object:
{
   ...
   * @returns {Message}
   */
  createNextMessage(runId, personas) {
    return {
      run_id: runId,
      event: EventTypes.PERSONAS_READY,
      from: this.id,
      to: NodeIds.SEARCH, // Next would be SearchNode
      payload: {
